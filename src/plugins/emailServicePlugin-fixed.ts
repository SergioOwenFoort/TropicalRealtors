...
export default function emailServicePlugin(): Plugin {
        ...
        if (req.method === 'POST') {
          ...
          req.on('end', async () => {
            try {
              ...
              // Use Supabase fallback approach since admin API is not working
              try {
                ...
                // Since admin API is broken, trigger Supabase's built-in password reset
                const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
                  redirectTo: `${req.headers.origin || 'http://localhost:5173'}/auth/update-password?fallback=true`
                });

                if (resetError) {
                  console.error('❌ Supabase reset error:', resetError);
                  throw new Error(`Supabase reset failed: ${resetError.message}`);
                }
                ...
                res.end(JSON.stringify({ 
                  success: true, 
                  message: 'Password reset process initiated. Please check your email for a new reset link from Supabase.',
                  method: 'supabase_fallback'
                }));

              } catch (fallbackError) {
                console.error('❌ Password reset failed:', fallbackError);
