...
export function VacationPropertyContact({ property, disableSticky = false }: VacationPropertyContactProps) {
  const { user } = useAuth();
  ...
  const handleMessageSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await MessageService.sendMessage({
        ...
        subject: `Interesse in: ${property.name}`
      });
            ...
            <div className="p-6">
              ...
              <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-700">
                  <strong>Eigendom:</strong> {property.name}
                </p>
