...
// Simulate the propertyService mapping function
function mapDbDataToProperty(dbData) {
  return {
    id: dbData.id,
    title: dbData.title,
...
async function testMapping() {
  console.log('🔍 Testing property mapping...');
  ...
  try {
    ...
    rawData.forEach((prop, index) => {
      console.log(`\n${index + 1}. ${prop.title}`);
      console.log(`   Raw property_type: ${prop.property_type}`);
      console.log(`   Raw type: ${prop.type}`);
      
      const mapped = mapDbDataToProperty(prop);
      console.log(`   Mapped type: ${mapped.type}`);
      console.log(`   Type check: ${mapped.type === 'koop' ? 'Te koop' : 'Te huur'}`);
    });
